...
def god_mod():
	if var1.get():
		try:
			Backend.main_strat('true', 'god_mod', selection_profile.get(), input_path.get())
		except:
			showerror('Error', 'ERROR: No such profile exists')
	else:
		try:
			Backend.main_strat('false', 'god_mod', selection_profile.get(), input_path.get())
		except:
			showerror('Error', 'ERROR: No such profile exists')
...
def checkinput():
	try:
		...
		try:
			Backend.main_strat(input_value.get(), selected_change.get(), selection_profile.get(), input_path.get())
		except:
			showerror('Error', 'ERROR: No such profile exists')
